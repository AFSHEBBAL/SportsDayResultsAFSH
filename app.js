...
/* Fixed organizer PIN, stored as a salted SHA-256 hash so it isn't sitting in the
   source as plain text. This only deters casually reading the code — anyone who
   copies this file could still brute-force a 4-digit PIN offline in seconds, so
   don't treat this as strong security for anything more sensitive than this board. */
const PIN_SALT = 'afs-sportsday-2026-';
const PIN_HASH = '0dedd9f6d8cb79aff0c2c6f8bf5f933113a1d2443b27fdc02ad877e50533f5a6';
async function sha256Hex(str){
  const buf = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(str));
  return Array.from(new Uint8Array(buf)).map(b => b.toString(16).padStart(2,'0')).join('');
}
async function checkPin(entered){
  const hash = await sha256Hex(PIN_SALT + entered);
...
function renderRacePanel(ev){
  ...
  return `
    <div class="race-panel">
      ...
      <p class="race-hint">Press Enter each time a runner finishes — times are captured for the lanes in order (Lane 1 first, then Lane 2, etc). You can also press a lane's number key directly if you need to capture out of order. Click a lane name to rename it before starting.</p>
    </div>`;
}
...
document.addEventListener('keydown', e => {
  ...
  if(e.key === 'Enter'){
    e.preventDefault();
    const nextIdx = rs.laneTimes.slice(0, rs.laneCount).findIndex(t => t === null);
    if(nextIdx !== -1) captureLane(ev, nextIdx);
    return;
  }
...
function openPinModal(){
  ...
  $('#pinInput').value = '';
  $('#pinErr').textContent = '';
  $('#pinDesc').textContent = 'Enter the organizer PIN to edit and publish results.';
